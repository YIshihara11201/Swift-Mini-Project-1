...
class FolderStructure {
    ...
    static func printFolders(for rootPath: String) -> Void{
        print(rootPath)
        printFoldersHelper(for: rootPath, currPath: "", depth: 0)
    }

    private static func printFoldersHelper(for parentPath: String, currPath: String, depth: Int, padding: String = "") -> Void {
        do {
            ...
            for (index, url) in directoryContents.enumerated() {
                ...
                // print "└─" for last element for a folder
                ...
                if isDirectory(at: parentPath + "/" + url) {
                    printFoldersHelper(for: parentPath + "/" + url, currPath: url, depth: depth+1, padding: padding + paddingChar)
                }
            }
        } catch {
            print(error)
